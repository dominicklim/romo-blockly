...
Maze.displayPegman = function(x, y, d) {
  ...
  pegmanIcon.setAttribute('y', Maze.SQUARE_SIZE * (y + 0.5) - Maze.PEGMAN_HEIGHT / 2 - 8);
...
Maze.isWall = function(direction) {
  ...
  if (Maze.pegmanD == (0 + direction)) {
    return Maze.pegmanX == (Maze.COLS - 1);
  }
  else if (Maze.pegmanD == constraint(1 + direction)) {
    return Maze.pegmanY == (Maze.ROWS - 1);
  }
  else if (Maze.pegmanD == constraint(2 + direction)) {
    return Maze.pegmanX == 0;
  }
  else if (Maze.pegmanD == constraint(3 + direction)) {
    return Maze.pegmanY == 0;
  }
  else
  {
    return false;
